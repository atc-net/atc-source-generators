...
Console.WriteLine($"  Features:UseRedisCache = {configuration["Features:UseRedisCache"]}");
Console.WriteLine($"  Features:UseAdvancedLogging = {configuration["Features:UseAdvancedLogging"]}");
Console.WriteLine($"  Features:UsePremiumFeatures = {configuration["Features:UsePremiumFeatures"]}");
Console.WriteLine();
...
// Note: When services have Condition parameter, IConfiguration must be passed
services.AddDependencyRegistrationsFromDependencyRegistration(configuration);
...
Console.WriteLine("\n13. Conditional Registration (Feature Flags):");
Console.WriteLine("Services can be registered conditionally based on configuration values.");
Console.WriteLine("This is useful for feature flags, environment-specific services, and A/B testing.\n");

using (var scope = serviceProvider.CreateScope())
{
    // Test cache service - registered based on Features:UseRedisCache config
    var cache = scope.ServiceProvider.GetRequiredService<ICache>();
    Console.WriteLine($"Cache Provider: {cache.ProviderName}");
    Console.WriteLine($"  (Registered based on Features:UseRedisCache = {configuration["Features:UseRedisCache"]})\n");
...
using (var scope = serviceProvider.CreateScope())
{
    try
    {
        var premiumService = scope.ServiceProvider.GetRequiredService<IPremiumFeatureService>();
        premiumService.ExecutePremiumFeature("Advanced Analytics");
        premiumService.ExecutePremiumFeature("AI-Powered Insights");

        Console.WriteLine($"\n✓ Premium features are available (Features:UsePremiumFeatures = {configuration["Features:UsePremiumFeatures"]})");
    }
    catch (InvalidOperationException)
    {
        Console.WriteLine($"✗ Premium features are not available (Features:UsePremiumFeatures = {configuration["Features:UsePremiumFeatures"]})");
        Console.WriteLine("  To enable, set Features:UsePremiumFeatures to true in appsettings.json");
    }
}

Console.WriteLine("\nBenefits of Conditional Registration:");
Console.WriteLine("  ✓ Feature Flags - Enable/disable features without code changes");
Console.WriteLine("  ✓ Environment-Specific - Different implementations for dev/staging/prod");
Console.WriteLine("  ✓ A/B Testing - Register different implementations based on configuration");
Console.WriteLine("  ✓ Cost Optimization - Disable expensive services when not needed");
Console.WriteLine("  ✓ Gradual Rollout - Safely test new implementations before full deployment");
